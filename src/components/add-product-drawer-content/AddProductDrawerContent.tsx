import { Alert, Autocomplete, Button, Container, Snackbar, Stack, TextField, Typography } from '@mui/material'
...
function AddProductDrawerContent() {
  ...
  if (categoryError) return <DefaultErrorMessage message="Error loading categories" />
      ...
      <Stack spacing={2}>
        ...
        <Stack direction={'row'} spacing={2}>
          <Autocomplete
            ...
            value={categoryData!.data!.filter((category) => productDetails.categoriesIds.includes(category.id))}
            onChange={(_, value) => {
              setProductDetails((prev) => ({
                ...prev,
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} justifyContent="center">
          <Button variant="text" color="primary" onClick={() => handleAddProduct()} fullWidth>
            Add Product
          </Button>

          <Button variant="text" color="error" onClick={() => handleCancelUpdate()} fullWidth>
            Cancel
          </Button>
